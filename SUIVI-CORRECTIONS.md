...
| `SOL-6` | 🟠 | **Solde SOL périmé après une transaction** et **aucun accès à la preuve depuis le hangar.** `getBalance` était lu immédiatement après la signature, avant confirmation réseau — et l'anti-rebond de 3 s bloquait la relecture suivante. Corrigé le 14/08 : `rafraichirApresTx()` relit à 0 / 2,5 / 7 s en forçant l'anti-rebond (appliqué aussi au don SOL et au paiement SKR). Ajouté : lien Solscan sur chaque vaisseau acheté dans le hangar, et `renderJournalTx()` rappelé après l'achat. | **à tester** |
| `SOL-7` | 🟡 | **Solde SOL affiché à 2 décimales** : 1,9987 devient « 2.00 », d'où l'impression d'un montant approximatif. Passé à 4 décimales (affichage + toast). | **à tester** |
| `SOL-8` | 🟠 | **La trésorerie est le wallet du joueur.** `TRESORERIE.adresse` = `DONS.adresse` = `AVhVM29hD…XBWSH`, l'adresse de test d'AzumiZeus. Un achat s'envoie donc à soi-même : la transaction est réelle et vérifiable, mais le solde ne bouge que des frais de réseau (~0,000005 SOL). Décision à prendre avant tout passage en mainnet : adresse de trésorerie distincte. | à faire |
| `UI-1` | 🟡 | **`quitterSplash is not defined`** : le splash est cliquable dès l'affichage du HTML, alors que le script de 3 Mo n'est évalué qu'ensuite — le clic levait une `ReferenceError` et se perdait. Corrigé le 14/08 : garde `typeof`, et le clic trop tôt est mémorisé puis honoré quand le script est prêt. **Préexistant, pas une régression** (vérifié au commit précédent). | **à tester** |
| `UI-2` | 🟡 | **Plein écran tenté sur ordinateur** : le navigateur le refuse sans geste utilisateur (`requestFullscreen: API can only be initiated by a user gesture` en boucle), et quand il l'accepte il faut sortir avec Échap. Réservé au tactile le 14/08. | **à tester** |
| `UI-3` | 🟡 | **Messages illisibles** : `toast()` durait 2,4 s par défaut — pas le temps de lire un déblocage. Porté à 3,8 s. | **à tester** |
| `UI-4` | 🟡 | **`navigator.vibrate` bloqué en boucle** dans la console : la démo de l'accueil déclenche des boss avant toute interaction, or le navigateur interdit la vibration tant que la page n'a pas été touchée. `vibrate()` attend maintenant le premier geste. | **à tester** |
| `UI-5` | 🟠 | **La démo affichait le panneau de fin de mission.** La démo est une vraie partie (pilote auto, 99 vies) : en bouclant un secteur elle arrivait dans `endGame()`, qui allait jusqu'au `modal-result`. Le garde `g.demo` existant ne couvrait que la sauvegarde, pas l'affichage. `endGame()` sort désormais immédiatement en démo via `arreterDemo()`. | **à tester** |
| `ECO-1` | 🟡 | **Grille de prix refondue** : 0,03 à 0,33 SOL (2,26 à 24,81 $ au cours de 75,19), rapport SKR/SOL uniforme à 25 000 pile. Deux décimales maximum, sinon le texte déborde des boutons du hangar. Le test `skr_sc` vérifie la cohérence des taux : baisser le SOL seul le casserait. | **à tester** |
| `ECO-2` | 🔴 | **Les vaisseaux payants donnent jusqu'à +80 % de dégâts** (`bonus` 1.00 → 1.95), en contradiction avec la décision « NFT cosmétiques stricts » et avec ce que le jeu affiche déjà. À faire : ramener `bonus` à 1.00 pour les 8 vaisseaux non mérités (Phantom compris, sinon le gratuit devient meilleur que les payants), garder l'échelle 1.28→1.95 sur les 6 mérités, et différencier les payants par `TIRS[]`. **À coordonner avec le rééquilibrage de la difficulté** : retirer la béquille de puissance sans baisser la difficulté rend le jeu injouable. Voir `docs/ECONOMIE-NFT.md`. | à faire |
| `ECO-3` | 🟡 | Vérifier **où `bonus` s'applique exactement** — dégâts par tir, cadence, ou les deux. Jamais vérifié. Prérequis de `ECO-2`. | à faire |
